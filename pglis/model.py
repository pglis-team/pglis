...
The model computes the differential flux J [MeV/n^-1 sr^-1 s^-1 m^-2] for a given nuclear species (atomic number Z) as a function of kinetic energy per nucleon (Ekn, MeV/n) and time (Unix timestamp, seconds).
...
def update_dataset(
    ...
    # normalise Z
    Z_list = (
        list(range(1, 29)) if Z is None else ([Z] if isinstance(Z, int) else list(Z))
    )
    # normalise polarity
    pol_list = (
        ["Apos", "Aneg"]
    ...
    for pol in pol_list:
        ...
        for z in Z_list:
            ...
            try:
                if verbose:
                    print(f"[pglis] Downloading {filename} ...", end=" ", flush=True)
...
def _check_and_update_dataset(verbose: bool = False) -> None:
    ...
    if latest is None:
        ...
        missing = any(
            not (
                Path(_HERE) / "data_products" / pol / f"pglis_{pol}_Z{z:02d}.csv"
            ).exists()
        ...
        if missing:
            ...
            if verbose:
                print(f"[pglis] Offline — using stored dataset version {stored}.")
        return
    ...
    if stored is None:
        if verbose:
            print(f"[pglis] First run — downloading dataset version {latest}...")
    else:
        if verbose:
            print(f"[pglis] New dataset version {latest} (had {stored}) — updating...")
...
def update_ssn(verbose: bool = False) -> bool:
    ...
    try:
        ...
        obs = {e["time-tag"]: e.get("smoothed_ssn") for e in _fetch(_SSN_OBSERVED_URL)}
        pred = {
            e["time-tag"]: e.get("predicted_ssn") for e in _fetch(_SSN_PREDICTED_URL)
        }
